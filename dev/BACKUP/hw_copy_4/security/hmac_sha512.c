...
security_status_e hmac_sha512_init(hmac_sha512_t *ctx, const uint8_t *key, uint32_t key_len)
{
    ...
    _SECURITY_CHECK_VALID_NOT_NULL(memset(ctx, 0x00, sizeof(hmac_sha512_t)));

    _SECURITY_VALID_RES(sha512_init(&ctx->sha_ctx));

    if (key_len > HMAC_SHA512_BLOCK_SIZE)
    {
        _SECURITY_VALID_RES(sha512(key, key_len, ctx->key));
    }
    else 
    {
    ...
    _SECURITY_VALID_RES(sha512_update(&ctx->sha_ctx, ipad_xor_arr, HMAC_SHA512_BLOCK_SIZE));
...
security_status_e hmac_sha512_update(hmac_sha512_t *ctx, const uint8_t *data, uint32_t data_len)
{
    ...
    _SECURITY_VALID_RES(sha512_update(&ctx->sha_ctx, data, data_len));
...
security_status_e hmac_sha512_finish(hmac_sha512_t *ctx, uint8_t *out)
{
    ...
    _SECURITY_VALID_RES(sha512_finish(&ctx->sha_ctx, out));

    _SECURITY_VALID_RES(sha512_init(&ctx->sha_ctx));
    _SECURITY_VALID_RES(sha512_update(&ctx->sha_ctx, opad_xor_arr, HMAC_SHA512_BLOCK_SIZE));
    _SECURITY_VALID_RES(sha512_update(&ctx->sha_ctx, out, HMAC_SHA512_HASH_SIZE));
    _SECURITY_VALID_RES(sha512_finish(&ctx->sha_ctx, out));
                ...
                const uint8_t *data, uint32_t data_len, uint8_t *out)
{
    ...
    _SECURITY_VALID_RES(hmac_sha512_init(&ctx, key, key_len));
    _SECURITY_VALID_RES(hmac_sha512_update(&ctx, data, data_len));
    _SECURITY_VALID_RES(hmac_sha512_finish(&ctx, out));
