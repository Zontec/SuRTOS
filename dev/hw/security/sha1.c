...
security_status_e sha1_init(sha1_t *ctx)
{
    ...
    _SECURITY_CHECK_VALID_NOT_NULL(memset(ctx, 0x00, sizeof(sha1_t)));
...
security_status_e sha1_finish(sha1_t *ctx, uint8_t *out)
{
    ...
    _SECURITY_VALID_RES(sha1_update(ctx, sha1_padding, padn));
    _SECURITY_VALID_RES(sha1_update(ctx, msglen, 8));
...
security_status_e sha1(const uint8_t *data, uint32_t data_len, uint8_t *out)
{
    ...
    _SECURITY_VALID_RES(sha1_init(&ctx));
    _SECURITY_VALID_RES(sha1_update(&ctx, data, data_len));
    _SECURITY_VALID_RES(sha1_finish(&ctx, out));
